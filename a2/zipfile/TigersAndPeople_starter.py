...
def SearchForPlan(some_plan,Left,Right,Boat,Backup):
    ...
    # print names, rather than function objects
    def print_plan(plan):
        print "Current plan:"
    ...
    # is the given move the last one in the plan
    def is_last_move(move):
        return some_plan[-1][0] == move

    # is the given move the second last one in the plan
    ...
    # if test_plan(plan) == -1, the result will be false
    # otherwise, true.
    def is_safe(plan):
    ...
    # search the cross child of the current move first
    def search_cross():
        # prevent cross looping
        ...
        # if it is safe to cross, we can cross
        if is_safe(new_plan):
            some_plan.append([cross, []])
            return True
        return False

    # search the swap child of the current node
    def search_swap():
        # prevent swap looping
        if is_last_move(swap_type) and is_second_last_move(swap_type):
            return False
        # print 'Trying to swap'

        # try to swap tigers first, its safer
        new_plan = some_plan[:]
        ...
        if is_safe(new_plan):
            ...
            # if there are no more tigers to swap, swap people
            new_plan.pop()
    ...
    # search the unloading child of the current node
    def search_unload():
        ...
        # try to unload tigers first
        # note, this will keep a single person in the boat to make sure
        # a person can drive the boat always
        new_plan = some_plan[:]
        ...
        if is_safe(new_plan):
            ...
            # if everyone is on the right,
            # the boat driver can unload
            if len(Left_Side) == 0:
                new_plan.pop()
    ...
    # search the load child of the current node
    def search_load():
        # only load on the left side
        if Boat.side:
            return False
